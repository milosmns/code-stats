...
class CodeReviewFeedbacksPostponedCalculatorTest {

  private val repos = listOf(Stubs.metrics.codeReviewFeedbackRepo1, Stubs.metrics.codeReviewFeedbackRepo2)
  private val calculate = { CodeReviewFeedbacksPostponedCalculator().calculate(repos) }
  ...
  @Test fun `calculation is correct per author`() {
    ...
    assertThat(valuesPerAuthor[Stubs.metrics.user2]).isEqualTo(3)
  ...
  @Test fun `calculation is correct per reviewer`() {
    ...
    assertThat(valuesPerReviewer[Stubs.metrics.user3]).isEqualTo(5)
  }

  @Test fun `calculation is correct per code review`() {
    val valuesPerCodeReview = calculate().perCodeReview

    assertThat(valuesPerCodeReview[repos[0].codeReviews[0]]).isEqualTo(2)
